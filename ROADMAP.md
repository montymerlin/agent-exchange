# Roadmap — Agentic Exchange Workshop

Where this project could go. Items here are aspirations, not commitments — a place to capture ideas, inspiration, and future directions without the pressure of a deadline.

When an item gets evaluated, the outcome is logged in [DECISIONS.md](DECISIONS.md). This creates a traceable pipeline from idea to decision.
...
- **Beginner module: What is agentic AI?** — Introductory materials covering the difference between conversational AI and agentic AI, with hands-on examples. `status: active`
- **Intermediate module: AI file system access** — How agents read, write, and organize files. Demonstrate with live Cowork examples. `status: active`
- **Workshop exercise: Scaffold your own project** — Guided exercise where participants run /init on their own folder using the agentic-scaffold-plugin. `status: active`
